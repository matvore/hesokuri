...
(def ^:dynamic *letmap-omitted-key*
  "When non-nil, maps generated by letmap will include an entry containing all
  'omitted' entries. This key of the entry is this value, and the value is a map
  of omitted entries. Code should not access omitted entries in normal
  circumstances. This functionality is provided to increase testability of code
  using :omit."
  nil)
     ...
     (loop [bindings (seq bindings)
       ...
       (cond
        ...
        `(let ~let-bindings
           (let [~map-name
                 (into ~map-expr
                       (if *letmap-omitted-key*
                         {*letmap-omitted-key* ~omit-expr}
                         {}))]
             ~@body))
